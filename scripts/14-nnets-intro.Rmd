...
With our dataset prepared, we can introduce the structure of a basic **feedforward neural network**, which consists of three main types of layers:

1.  **Input Layer:** - This layer consists of **784 neurons** (one for each pixel in the input image).
...
Below is an example of how the softmax function works. Picture we have 10 outputs, one 
for each possible class. These will initially be in logit form. 

```{r}

# ── Softmax numeric example: EMNIST output layer (10 classes: digits 0–9) ──────

# Raw logits from the final dense layer (one forward pass)
logits <- c(-1.2, 0.4, 2.1, 0.8, -0.3, 1.5, 0.2, -0.9, 3.1, 0.6)
names(logits) <- paste0("digit_", 0:9)

# Step 1: Exponentiate 
exp_logits <- exp(logits)

#  Step 2: Normalise (divide by partition function Z) 
...
# ── Cross-entropy loss: 10 observations, EMNIST digit classes (0–9) ────────────

# True labels (one per observation)
true_labels <- c(3, 7, 0, 5, 2, 9, 1, 6, 4, 8)

# Simulated softmax output: 10 obs × 10 class probabilities
# Each row sums to 1 (as if from a softmax layer)
raw <- matrix(runif(100), nrow = 10, ncol = 10)
probs <- raw / rowSums(raw)   # row-normalise → valid probability distributions
colnames(probs) <- paste0("digit_", 0:9)

# ── Cross-entropy per observation: -log(p_true) ────────────────────────────────
# Extract the predicted probability assigned to the correct class for each row
p_true <- map2_dbl(
  seq_len(nrow(probs)),
  true_labels + 1L,          # +1: R is 1-indexed, digits are 0-indexed
  \(row, col) probs[row, col]
)
...
cat("\nMean cross-entropy loss:", round(mean(cross_entropy), 4), "\n")
```
