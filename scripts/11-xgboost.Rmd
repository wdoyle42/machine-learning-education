...
# Sample 20 units from dataset `ou`


sample_data <- ou%>%
  sample_n(100)%>%
  mutate(y=ifelse(result=="passed",1,0))%>%
  select(-result,-id_student,-region,-imd_band,-highest_education,-age_band,-code_module)
...
for (i in 1:num_iterations) {
  ...
  sampled_data$residuals <- sampled_data$y - sampled_data$prediction
