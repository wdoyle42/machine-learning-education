...
pca_df <- as.data.frame(pca_result$x[, 1:2])  # Get first 2 principal components
...
for (i in 1:100) {
  updated <- backprop_step(X = synth$X, Y = synth$Y, network = network, lr = 0.1)
...
```{r}
loss_history<-data.frame(loss_history,x=1:100)
```
